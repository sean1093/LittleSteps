...
> 日期：2026-09-03
> 定位：LittleSteps 家族的第六個服務。回答一個家長每週都在問、而政府資料答得出來但答得不好懂的問題：**我這個縣市、我小孩這個年齡，這週要小心什麼。**
...
| 燈 | 文案 | 條件 | 白話 |
|---|---|---|---|
| ↑↑ | 明顯升高 | `ratio >= 1.88` | 對應 P90：歷史上只有十分之一的週這麼高 |
| ↑ | 升高 | `1.33 <= ratio < 1.88` | 對應 P75 |
| → | 持平 | `0.77 <= ratio < 1.33` | P25–P75 |
| ↓ | 下降 | `ratio < 0.77` | 對應 P25 |
| — | 近期無個案 | `trendBase === 0 && rate === 0` | 基線為零，比值無意義。2026 W34 在分母 ≥ 1,000 的 330 格中有 **13 格**如此，全部是水痘 |
| ⚑ | 出現個案 | `trendBase === 0 && rate > 0` | 從零變成有是資訊，但不能算成倍數 |
...
- `trendBase` 為 0（前 8 週皆無個案）時，`ratio` 與 `geoRatio` 皆為 `null`，燈號改用 §4.5 的「近期無個案」／「出現個案」。
...
│ [0-2 歲] [3-6 歲] [7-12 歲]                   │
├──────────────────────────────────────────────┤
│ 腸病毒         ↑↑ 明顯升高   169/萬   35 人次  │
│ 手足口病       ↑↑ 明顯升高    72/萬   15 人次  │
│ 疱疹性咽峽炎   ↑  升高        97/萬   20 人次  │
│ 類流感         →  持平       159/萬   33 人次  │
│ 腹瀉           →  持平       251/萬   52 人次  │
│ 水痘           —  近期無個案     —     0 人次  │
├──────────────────────────────────────────────┤
│ 這是健保門診就診統計，不是確診數，不能當診斷依據 │
└──────────────────────────────────────────────┘
```

- 六張卡用既有 `.card-tap`；排序固定（不依燈號重排），因為家長是在找特定的病，位置跳動會讓人找不到。
...
- 抽屜用既有 `ModalFrame` + `motion.ts` 的 `sheet` / `backdrop`，`max-h-[85vh] overflow-y-auto`。內容依序是：8 週 sparkline（inline SVG，`viewBox` + `w-full`，無固定像素寬高）、與全國同週的比較（`geoRatio`）、分母與樣本可靠度說明、前 5 年同週脈絡（附 NPI 註記）、疾管署該病介紹連結。
- 率、人次、分母三者都要出現。只給率會被問「那是幾個人」，只給人次會被跨縣市誤比。
- 圖示只出現在燈號與定位鈕上——燈號是狀態指示、定位鈕的圖示是它的全部標籤，兩者都符合 CLAUDE.md 的「圖示要自己掙到位置」。標題旁不放圖示。
...
| `14 < age <= 35 天` | 板照常顯示（stale beats blank），加一條「資料可能未更新」 |
| `age > 35 天` | **燈號全部收起**，只留數字，顯示「這份資料已超過一個月未更新，請以疾管署 NIDSS 為準」並連出去 |
| 檔案抓不到 / 解析失敗 | `EmptyState` 說明資料暫時無法取得並連向 NIDSS。不顯示空板，也不假裝有資料 |

門檻的由來：更新週期是 7 天，14 天等於連續兩週沒進來（異常但可能是上游延遲）；35 天等於五週，那時候的「上週」已經與現況無關，一個可能已經錯的燈比沒有燈更糟。
...
| `src/littleguard/data/diseases.test.ts` | 六筆說明的 `name` / 傳染 / 就醫 / `sourceUrl` / `verifiedOn` 皆非空 |
| `src/littleguard/utils/radar.test.ts` | 燈號邊界（1.879 → ↑、1.88 → ↑↑、0.769 → ↓、0.77 → →）；`trendBase === 0` 的兩種狀態；`small` 不亮燈；`insufficient` 不給數字；新鮮度三段門檻（14 / 35 天邊界） |
| 校準測試（同上檔） | `RADAR_THRESHOLDS` 常數與 JSON `calibration` 差距 ≤ 0.05 |
| `src/littleguard/pages/RadarPage.test.tsx` | 切換 chip 換內容；抽屜開關；注入過期 JSON 時出現降級訊息且燈號消失；`insufficient` 格顯示「資料不足」；抓檔失敗顯示 `EmptyState` |
...
- **不做醫療建議**。抽屜只寫「哪些狀況該就醫」並連向疾管署。
- **不做地圖**。22 顆 chip 已經覆蓋全國，加 Leaflet 只會讓 12 KB 的板變重。
- **不做推播通知**。全 repo 沒有通知機制，而週更資料的推播價值不足以引入一套。
